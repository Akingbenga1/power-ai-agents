...
def create_specialized_agents():
    ...
    pdf_producer_agent = Agent(
        ...
        instructions="""You are a PDF Producer AI. You specialize in creating and formatting PDF documents from provided content.

        **IMPORTANT WORKFLOW:**
        You work in coordination with other agents:
        1. Content will be generated by the Content Writer agent first
        2. You receive the generated content and create the PDF document
        3. You focus ONLY on PDF creation and formatting, not content generation

        **CORE FUNCTIONALITY:**
        When you receive content to convert to PDF, you MUST:
        1. Use the create_pdf_document() function with the provided content
        ...
        **PDF CREATION PROCESS:**
        - You will receive pre-generated content from the Content Writer agent
        - Call create_pdf_document(generated_content) with the content provided to you
        - The function will automatically:
          * Extract the title and format the content properly
        ...
        **IMPORTANT:** 
        - You focus ONLY on PDF creation and formatting
        - Content generation is handled by other specialized agents
    ...
    async def delegate_task(self, agent_name, user_prompt):
        ...
        try:
            ...
            if agent_name == "PDF Producer":
                print(f"[DEBUG] PDF Producer agent activated - generating content from user request...")
                ...
                generated_content = generated_result.final_output.strip()  # Ensure we get the string output

                # Step 3: Use the generated content to create the PDF
                pdf_result = create_pdf_document(generated_content)
